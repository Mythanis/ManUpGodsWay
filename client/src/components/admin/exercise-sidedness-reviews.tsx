...
import { CheckCircle, XCircle, ChevronLeft, ChevronRight, Search, AlertTriangle } from "lucide-react";
...
function SidednessBadge({ value, size = "sm" }: { value: SidednessValue; size?: "sm" | "xs" }) {
  const base = SIDEDNESS_COLORS[value] ?? "bg-gray-100 text-gray-800 border-gray-300";
  return (
    <span
...
export default function ExerciseSidednessReviews() {
  ...
  const queryKey = ["/api/admin/exercise-sidedness-reviews", status, lowConfidenceOnly, debouncedSearch, page];
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        {status === "pending" && total > 0 && (
          <Button
            ...
            onClick={() => {
              if (window.confirm(`Approve all ${total} pending rows using Claude's proposed value? This writes to exercises.sidedness.`)) {
      ...
      {!isLoading && rows.length === 0 && (
        <div className="text-center py-12 text-sm text-gray-400 italic">
          {status === "pending"
            ? "No pending reviews. Run classify-exercise-sidedness script to populate the queue."
            : `No ${status} reviews.`}
        </div>
      )}
              ...
              <CardContent className="py-3 px-4">
                <div className="flex items-start gap-3 flex-wrap">
                  ...
                  <div className="flex-1 min-w-[180px]">
                    ...
                    {row.confidence === "low" && (
                      <span className="inline-flex items-center gap-0.5 text-[10px] text-amber-600 font-semibold mt-1">
                        <AlertTriangle className="h-2.5 w-2.5" />
                        Ambiguous — double-check
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-2 text-xs text-gray-500 shrink-0">
                    <span>Current:</span>
                    <SidednessBadge value={(row.currentSidedness ?? "bilateral") as SidednessValue} size="xs" />
                    <span>→</span>
                    <span>Proposed:</span>
                  ...
                  {status === "approved" && (
                    <div className="flex items-center gap-2 text-xs text-green-700 font-semibold shrink-0">
                      ...
                      <SidednessBadge value={(row.approvedSidedness ?? row.proposedSidedness) as SidednessValue} size="xs" />
                      {row.reviewedAt && (
