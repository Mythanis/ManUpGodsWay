...
import {
  User, Camera, Upload, X, Check, Bell, Scale, FileText,
  Crown, Shield, BookOpen, Video, Users, Sword,
  CalendarClock, CheckCircle, ChevronRight
} from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-black flex items-center justify-center p-4">
      <div
        ...
        style={{ background: "#0d0d0d", border: "2px solid #FDD000", boxShadow: "6px 6px 0px 0px rgba(253,208,0,0.25)" }}
      >
        {children}
      </div>
    </div>
...
// ─── Step header ────────────────────────────────────────────────────────────
function StepHeader({ icon, label, title, subtitle }: { icon: React.ReactNode; label: string; title: string; subtitle: string }) {
  return (
    <div className="text-center px-6 pt-4 pb-2">
      ...
      <p className="text-xs font-black uppercase tracking-[0.2em] mb-1" style={{ color: "#FDD000" }}>{label}</p>
      <h3 className="text-xl font-black text-white uppercase tracking-tight">{title}</h3>
      <p className="text-sm mt-1" style={{ color: "rgba(255,255,255,0.5)" }}>{subtitle}</p>
    </div>
  );
}
...
export function UserSetupWizard({ onComplete }: { onComplete: () => void }) {
  ...
  const updateProfileMutation = useMutation({
    mutationFn: (data: SetupData) =>
      apiRequest("POST", "/api/profile/setup", { ...data, isProfileComplete: true }),
    onSuccess: async () => {
      try {
  ...
  const handleAcceptNotificationPermission = async () => {
    ...
    if ("Notification" in window) {
      ...
      if (permission === "granted") {
        ...
        toast({ title: "Notifications Enabled", description: "You'll receive prayer time alerts." });
      } else {
        setSetupData({ ...setupData, prayerPermissionsGranted: false });
        toast({ title: "Notifications Not Enabled", description: "You can enable these later in settings." });
      }
    } else {
      setSetupData({ ...setupData, prayerPermissionsGranted: false });
    }
  ...
  // ── Shared footer ──────────────────────────────────────────────────────────
  const Footer = ({ onBack, onNext, nextLabel, nextDisabled }: {
    onBack?: () => void;
    onNext: () => void;
    nextLabel?: string;
    ...
    <div className="px-6 pb-6 pt-2 flex gap-3">
      ...
      <Button
        ...
        style={{ background: "#FDD000", border: "2px solid #000", boxShadow: "2px 2px 0px 0px rgba(0,0,0,1)" }}
      >
        {nextLabel || "Continue"}
  ...
  if (step === 1) return (
    <WizardCard>
      ...
      <div className="px-6 py-4 space-y-4">
        <div>
          <Label htmlFor="firstName" className="text-white font-bold text-xs uppercase tracking-wide">First Name *</Label>
          <Input
            id="firstName"
        ...
        <div>
          <Label htmlFor="lastName" className="text-white font-bold text-xs uppercase tracking-wide">Last Name *</Label>
          <Input
            id="lastName"
      ...
      <Footer onNext={handleNext} nextDisabled={!setupData.firstName.trim() || !setupData.lastName.trim()} />
    </WizardCard>
  );
    ...
    <WizardCard>
      ...
      <div className="px-6 py-4 flex flex-col items-center gap-4">
        {profileImagePreview ? (
          ...
          >
            <Upload className="w-8 h-8 mb-1" style={{ color: "rgba(255,255,255,0.3)" }} />
            <span className="text-xs" style={{ color: "rgba(255,255,255,0.3)" }}>Upload Photo</span>
          </div>
        )}
    ...
    <>
      <WizardCard>
        ...
        <div className="px-6 py-4 space-y-3">
          ...
          >
            <div className="flex items-center justify-between p-4">
              <div className="flex items-start gap-3">
                <CalendarClock className="w-5 h-5 mt-0.5 flex-shrink-0" style={{ color: "#FDD000" }} />
                <div>
              ...
              <Switch checked={dailyReminderEnabled} onCheckedChange={setDailyReminderEnabled} />
            </div>
            {dailyReminderEnabled && (
              <div className="px-4 pb-4 pt-1" style={{ borderTop: "1px solid #1e1e1e" }}>
                <p className="text-xs mb-2" style={{ color: "rgba(255,255,255,0.4)" }}>Remind me at:</p>
                <Select value={dailyReminderTime} onValueChange={setDailyReminderTime}>
                  ...
                  <SelectContent>
                    {Array.from({ length: 48 }, (_, i) => {
                      ...
                      return <SelectItem key={value} value={value}>{`${displayHour}:${mm} ${ampm}`}</SelectItem>;
                    })}
                  </SelectContent>
                </Select>
              </div>
          ...
          >
            <div className="flex items-start gap-3">
              <Bell className="w-5 h-5 mt-0.5 flex-shrink-0" style={{ color: "#FDD000" }} />
              <div>
                <p className="text-white font-bold text-sm">Prayer Timer Alerts</p>
                <p className="text-xs mt-0.5" style={{ color: "rgba(255,255,255,0.45)" }}>
          ...
          <div
            ...
            style={{ background: "rgba(253,208,0,0.06)", border: "1px solid rgba(253,208,0,0.15)" }}
          >
            <CheckCircle className="w-4 h-4 flex-shrink-0 mt-0.5" style={{ color: "#FDD000" }} />
            <p className="text-xs" style={{ color: "rgba(255,255,255,0.45)", lineHeight: 1.6 }}>
              You can change any of these preferences anytime from your profile settings.
            </p>
          </div>
      ...
      <Dialog open={showNotificationDialog} onOpenChange={setShowNotificationDialog}>
        <DialogContent className="max-w-sm bg-[#0d0d0d] border-2 border-[#FDD000] rounded-sm">
          ...
          <p className="text-sm" style={{ color: "rgba(255,255,255,0.6)", lineHeight: 1.7 }}>
            Your device will ask for permission to send notifications. Tap <strong className="text-white">Allow</strong> to enable prayer timer alerts.
          </p>
          <div className="flex gap-3 pt-2">
            <Button variant="outline" onClick={() => setShowNotificationDialog(false)} className="flex-1 rounded-sm border-white/20 text-white/60">
              Cancel
            </Button>
            <Button
    ...
    <>
      <WizardCard>
        ...
        <div className="px-6 py-4 space-y-3">
          ...
          >
            ...
            >
              {acceptedTerms
                ...
                : <Scale className="w-4 h-4" style={{ color: "rgba(255,255,255,0.4)" }} />
              }
            </div>
            <div className="flex-1">
              <p className="text-white font-bold text-sm">Terms &amp; Conditions</p>
              <p className="text-xs mt-0.5" style={{ color: acceptedTerms ? "#FDD000" : "rgba(255,255,255,0.4)" }}>
                {acceptedTerms ? "✓ Accepted" : "Tap to read and accept"}
              </p>
            </div>
            {!acceptedTerms && <ChevronRight className="w-4 h-4" style={{ color: "rgba(255,255,255,0.3)" }} />}
          </div>
          ...
          >
            ...
            <div className="flex-1">
              ...
              <p className="text-xs mt-0.5" style={{ color: acceptedPrivacy ? "#FDD000" : "rgba(255,255,255,0.4)" }}>
                {acceptedPrivacy ? "✓ Accepted" : "Tap to read and accept"}
              </p>
            </div>
            {!acceptedPrivacy && <ChevronRight className="w-4 h-4" style={{ color: "rgba(255,255,255,0.3)" }} />}
          </div>
        ...
        <Footer
          onBack={handleBack}
          onNext={handleNext}
          nextLabel={updateProfileMutation.isPending ? "Setting up..." : "Complete Setup"}
          nextDisabled={!acceptedTerms || !acceptedPrivacy || updateProfileMutation.isPending}
        />
      </WizardCard>
            ...
            <div className="text-white space-y-4">
              ...
              {TERMS_SECTIONS.map((section) => (
                <section key={section.id}>
                  <h2 className="text-base font-bold uppercase mb-2" style={{ color: "#FDD000" }}>{section.heading}</h2>
                  ...
                  {section.subsections && (
                    <div className="space-y-3 mt-2">
                      {section.subsections.map((sub) => (
                        <div key={sub.id}>
                          <h3 className="text-sm font-semibold mb-1" style={{ color: "#FDD000" }}>{sub.heading}</h3>
              ...
              <p className="text-sm font-semibold italic leading-relaxed" style={{ color: "#FDD000" }}>{TERMS_CLOSING}</p>
            </div>
          </ScrollArea>
          <div className="pt-4" style={{ borderTop: "1px solid #222" }}>
            <Button
              onClick={() => { setAcceptedTerms(true); setShowTermsDialog(false); }}
              className="w-full text-black font-black uppercase rounded-sm py-3"
              style={{ background: "#FDD000" }}
            >
              <Check className="w-5 h-5 mr-2" /> I Accept the Terms &amp; Conditions
            </Button>
          </div>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent className="max-w-lg max-h-[80vh] bg-[#0d0d0d] border-2 border-[#FDD000] rounded-sm">
          ...
          <ScrollArea className="h-[50vh] pr-4">
            <div className="text-white space-y-4">
              ...
              {[
                { title: "1. Introduction", body: "ManUp God's Way (\"we,\" \"us,\" or \"our\") respects your privacy. This Privacy Policy explains how we collect, use, store, and protect your information when you use our mobile application or related services (\"the App\")." },
                { title: "2. Information We Collect", body: "We collect only information you voluntarily provide:", bullets: ["First name", "Last name", "Email address", "Testimony content (text)", "Uploaded pictures/images"] },
                { title: "3. How We Use Your Information", body: "We use your information solely to:", bullets: ["Operate and support the App", "Display or manage submitted testimonies and content", "Communicate with you", "Send ministry-related emails", "Maintain safety, integrity, and functionality of the App"], closing: "We do not use your data for advertising, tracking, or profiling." },
                { title: "4. Third-Party Services", body: "We do not sell or share your data. We use Mailchimp (email only) and Replit (hosting infrastructure) as service providers." },
                { title: "5. Data Retention", body: "We retain your data only as long as needed for ministry and app operations unless legally required otherwise." },
                { title: "6. Security", body: "We use commercially reasonable administrative, technical, and organizational safeguards. No system is 100% secure." },
                { title: "7. Your Rights & Data Deletion", body: "You may request a copy of your data or full deletion by emailing info@manupgodsway.org." },
                { title: "8. Children's Privacy", body: "Not intended for children under 13." },
                { title: "9. No Tracking", body: "We do not track users across apps or websites." },
                { title: "10. Changes", body: "We may update this policy. Continued use means acceptance." },
                { title: "11. Contact", body: "info@manupgodsway.org" },
              ].map((s) => (
                <section key={s.title}>
                  <h2 className="text-base font-bold uppercase mb-2" style={{ color: "#FDD000" }}>{s.title}</h2>
                  ...
                  {s.closing && <p className="text-gray-300 text-sm leading-relaxed mt-2">{s.closing}</p>}
                </section>
              ))}
            </div>
          </ScrollArea>
          <div className="pt-4" style={{ borderTop: "1px solid #222" }}>
            <Button
              onClick={() => { setAcceptedPrivacy(true); setShowPrivacyDialog(false); }}
              className="w-full text-black font-black uppercase rounded-sm py-3"
              style={{ background: "#FDD000" }}
            >
  ...
  if (step === 5) return (
    <WizardCard>
      ...
      <div className="px-6 pt-4 pb-2 text-center">
        ...
        <p className="text-xs font-black uppercase tracking-[0.2em] mb-1" style={{ color: "#FDD000" }}>
          You're In, Soldier
        </p>
        ...
        <p className="text-sm mt-1" style={{ color: "rgba(255,255,255,0.5)" }}>
          Start your free 7-day trial — no charge until it ends, no credit card required.
        </p>
      </div>

      <div className="px-6 py-3 space-y-2">
        {[
          { icon: <BookOpen className="w-4 h-4" style={{ color: "#FDD000" }} />, text: "Full Bible study library + 365-day reading plans" },
          { icon: <Video className="w-4 h-4" style={{ color: "#FDD000" }} />, text: "Complete video & podcast library" },
          { icon: <Users className="w-4 h-4" style={{ color: "#FDD000" }} />, text: "Community, War Room & War Groups" },
          { icon: <Sword className="w-4 h-4" style={{ color: "#FDD000" }} />, text: "Weekly challenges & rank progression" },
          { icon: <Shield className="w-4 h-4" style={{ color: "#FDD000" }} />, text: "Under Fire accountability network" },
        ].map((item, i) => (
          <div
            key={i}
      ...
      {canInstall && (
        <div
          ...
          style={{ background: "#111", border: "1px solid rgba(253,208,0,0.2)" }}
        >
          <div className="flex-1">
