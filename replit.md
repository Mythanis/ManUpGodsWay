...
- **Terms & Conditions System**: Versioned Terms of Use (current version `2026-04-28`, 17-section legal document) in `shared/termsContent.ts`. All three surfaces (Terms page, signup wizard dialog, forced re-agreement gate) import from one canonical source. `terms_acknowledgements` DB table audits every acceptance (source: signup/forced_reagreement/settings_reaccept). Users with a mismatched `accepted_terms_version` in the `users` table see a full-screen gate on next login with scroll-to-bottom detection before "I Agree" enables. Allow-listed bypass paths: `/profile`, `/terms-conditions`, `/privacy-policy`, `/privacy-security`.
