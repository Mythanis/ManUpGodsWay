...
- **Live Streaming (Mux)**: Native in-app live streaming powered by Mux. Admins create a stream in the admin panel which calls the Mux API and returns RTMP credentials (server URL + stream key). Admin can optionally add YouTube and/or Facebook stream keys to simulcast simultaneously. Admin clicks "Go Live" to show a live banner to all members. Members tap the banner to watch inside the app at `/live` via an HLS player (hls.js). Members can dismiss the live banner via an X button (state persisted in localStorage per stream ID). Mux webhook at `POST /api/mux/webhook` auto-updates stream status when broadcaster connects/disconnects; also handles `video.asset.ready` to automatically save stream recordings as Video records. Manual "Save Recording" button appears on ended stream cards in admin (`POST /api/live-streams/:id/save-recording`) — queries Mux for ready assets and creates a video record with HLS URL + Mux thumbnail. Backend: `server/mux.ts` wraps `@mux/mux-node`. Requires paid Mux plan (billing at dashboard.mux.com). Credentials: `MUX_TOKEN_ID`, `MUX_TOKEN_SECRET` in Replit Secrets.
